...
GEOCODE_TIMEOUT = 10  # Timeout for geocoding requests
...
async def get_forecast(latitude: float, longitude: float) -> str:
    ...
    # NWS API requires latitude, a longitude format with up to 4 decimal places
    ...
    if points_data is None or 'properties' not in points_data:
        return f'Unable to retrieve NWS grid point information for {latitude:.4f},{longitude:.4f}.'

    # Extract forecast URLs from the grid point data
...
async def get_forecast_by_city(city: str, state: str) -> str:
    ...
    try:
        ...
        return f"An unexpected error occurred while finding coordinates for '{city_name}, {state_code}' with exception '{ex}'."
